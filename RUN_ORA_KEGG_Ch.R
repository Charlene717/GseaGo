...
##### Prepare Input #####
  ...
  ids <- bitr(names(original_gene_list), fromType = "SYMBOL", toType = "ENTREZID", OrgDb="org.Mm.eg.db") # remove duplicate IDS (here I use "ENSEMBL", but it should be whatever was selected as keyType)
...
##### Plot Chart #####
  ...
  mmu.p1 <- pathview(gene.data=gene_list, pathway.id="05235", species = kegg_organism, gene.idtype=gene.idtype.list[1])
  
  # Produce a different plot (PDF) (not displayed here)
  mmu.p2 <- pathview(gene.data=gene_list, pathway.id="05235", species = kegg_organism, gene.idtype=gene.idtype.list[1], kegg.native = F)
